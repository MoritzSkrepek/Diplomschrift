...
Dieser Abschnitt beschreibt, wie die Webseite gestartet und verwendet wird. Der gesamte Code der Webseite ist in einer
Datei namens \textit{Level1-FrontEnd/index.html} enthalten. Im Folgenden wird erklärt, wie diese Datei ausgeführt wird,
um die Webseite im lokalen Netzwerk bereitzustellen.

\subsubsection{Starten der Webseite}

Um die Webseite zu starten, wird ein lokaler Server benötigt. Wir haben uns für die Verwendung von \textit{http-server}
entschieden, aber die Wahl des Servers ist nicht relevant, solange die Webseite im lokalen Netzwerk erreichbar ist. Zur
Installation von \textit{http-server} auf einem Windows-Gerät kann der Befehl \textit{npm install http-server -g} in der
...
Befehl \textit{http-server} und den Flags \textit{-c -l -f} gestartet werden.

Die Flags haben folgende Bedeutungen:
- \texttt{-c}: Verhindert, dass die Webseite gecached wird.
- \texttt{-l}: Startet die Webseite auf localhost:8080.
- \texttt{-f}: Öffnet die Webseite automatisch im Browser.

Der Befehl lautet dann: \textit{http-server -c -l -f index.html}.

TODO: Flags mit -c-1 -f testen und die Beschreibung entsprechend aktualisieren. CORS-Plugin erklären, wenn erforderlich.

Der Server wird standardmäßig auf dem Port 8080 gestartet. Falls ein anderer Port gewünscht ist, kann dieser mit dem
Flag \textit{-p} und der gewünschten Portnummer geändert werden.

Nach dem Starten der Webseite kann der Benutzer sie im Browser seiner Wahl mit der IP-Adresse des Geräts aufrufen. Die
IP-Adresse des Geräts kann mit dem Befehl \textit{ipconfig} in der Kommandozeile gefunden werden.

TODO: Erklären, wie man das CORS-Plugin verwendet, falls keine Änderungen nach dem Test festgestellt wurden.

Nach dem erfolgreichen Starten der Webseite muss als Erstes die IP-Adresse der HoloLens eingegeben werden. Dies erfolgt
über das Zahnrad in der oberen linken Ecke der Webseite. Die IP-Adresse wird in das Eingabefeld eingetragen und mit dem
Button \textit{Connect} bestätigt. Erst nachdem die IP-Adresse eingetragen wurde, können Nachrichten versendet werden.
...
wird in der Combobox \textit{Rechtes Gerät} oder \textit{Linkes Gerät}, relativ zum Träger der Brille, ausgewählt.

Nach der erfolgreichen Konfiguration der Webseite kann der Benutzer Nachrichten versenden.

\subsubsection{Bedienung der Webseite}

Nun, da die Webseite läuft, wie wird sie verwendet?
