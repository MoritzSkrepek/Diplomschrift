...
% einfügen aller modelle in kleiner tabellenfrom
\begin{figure}[H]
    \centering
...
Die Modellierung der Energydose begann mit einem einfachen Zylinder, der dann anhand eines Referenzbildes geformt wurde. Besonderes Augenmerk wurde auf die Textur gelegt, wobei ein Redbull-Label als Referenz diente.
...
\subsection{Aufbau von Nachrichtenaustausch Anwendungsscenario} \marginpar{\small$(\rightarrow)$ Schoditsch}
...
\subsection{Kabelerkennung} \marginpar{\small$(\rightarrow)$ Schoditsch}
...
\subsection{Start der Szene} \marginpar{\small$(\rightarrow)$ Schoditsch}
...
\subsection{Foto aufnahme und verarbeitung} \marginpar{\small$(\rightarrow)$ Schoditsch}
Dieses Kapitel beschäftigt sich mit dem aufnehmen und verarbeiten von dem Foto des Roten Kabels
\subsubsection{\label{sec:Photocapture}Photocapture Klasse}\marginpar{\small$(\rightarrow)$ Schoditsch}
...
\subsubsection{Foto der Umgebung aufnehmen} \marginpar{\small$(\rightarrow)$ Schoditsch}
...
\subsubsection{\label{sec:JobSystem}Unity Job System} \marginpar{\small$(\rightarrow)$ Schoditsch}
...
\subsubsection{\label{sec:Vector2}Vector2 Klasse}\marginpar{\small$(\rightarrow)$ Schoditsch}
...
\subsubsection{Fotoverarbeitung} \marginpar{\small$(\rightarrow)$ Schoditsch}
...
\subsubsection{Kabel finden} \marginpar{\small$(\rightarrow)$ Schoditsch}
...
\subsection{\label{sec:Coroutines}Coroutines} \marginpar{\small$(\rightarrow)$ Schoditsch}
...
\subsection{Bewegung des Paketes} \marginpar{\small$(\rightarrow)$ Schoditsch}
...
\subsubsection*{Definition eines Rays} \marginpar{\small$(\rightarrow)$ Schoditsch}
...
\subsubsection{\label{sec:ARRaycastHit}ARRaycastHit Klasse}\marginpar{\small$(\rightarrow)$ Schoditsch}
...
\subsubsection{Erstellung und Positionierung des Paketes} \marginpar{\small$(\rightarrow)$ Schoditsch}
...
\subsubsection{Bewegung des Paketes} \marginpar{\small$(\rightarrow)$ Schoditsch}
...
\subsection{Anzeigen von Informationen der Nachricht} \marginpar{\small$(\rightarrow)$ Schoditsch}
...
\subsection{Debugging Optionen} \marginpar{\small$(\rightarrow)$ Schoditsch}
...
\subsubsection{\label{sec:PlaneDebug}Anzeigen der Kabelerkennung} \marginpar{\small$(\rightarrow)$ Schoditsch}
...
\subsubsection{Debug Funktion von Raycasts} \marginpar{\small$(\rightarrow)$ Schoditsch}
...
\end{lstlisting}
\subsubsection{Debug Funktion von der Kamera Benutzung} \marginpar{\small$(\rightarrow)$ Schoditsch}
...
\subsection{Andere Versuche / Probleme bei der Programmierung} \marginpar{\small$(\rightarrow)$ Schoditsch}
...
\subsubsection{Kabel Erkennung Version 1} \marginpar{\small$(\rightarrow)$ Schoditsch}
...
\subsection{\label{sec:testFramework}\textit{Unity Test Framework}}
...
%TODO: SS von Test-Runner Fester einfügen
...
Ein Testfall im \textit{Unity Test Framework} folgt einem bestimmten Aufbau, um sicherzustellen, dass die Funktionalität, die getestet werden soll, korrekt überprüft wird. Im obigen Beispiel (Code \ref{code:zeroTest}) ist ein Testfall dargestellt, der sicherstellt, dass der maximale Wert des Rucksacks korrekt berechnet wird, wenn keine Gegenstände im Rucksack vorhanden sind. Der Testfall besteht aus drei Hauptteilen: \textit{Arrange, Act} und \textit{Assert}.
...
\begin{lstlisting}[style=csharp, caption={Codeabschnitt: Performance Messung}, label={code:performanceTest}]
    [Test]
...
Anzahl der Gegenstände	Dauer in ms
50	1
100	0
150	1
200	2
...
Nach dem Test können einige Schlussfolgerungen gezogen werden:
\begin{itemize}
