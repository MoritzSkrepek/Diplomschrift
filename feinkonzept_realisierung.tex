...
Unity bietet mit \textit{Prefabs} eine äußerst praktische Funktion zur Erstellung und Wiederverwendung von Game-Objekten.
Prefabs sind modulare, wiederverwendbare Elemente, die als Blaupausen für die Konstruktion von Game-Objekten dienen. Sie
ermöglichen Entwicklern, spezifische Objekte oder Objektstrukturen zu erstellen und zu katalogisieren. Diese Muster können
dann in unterschiedlichen Szenarien oder Projekten repliziert werden, was eine effiziente Wiederverwendung und Konsistenz
über verschiedene Projektumgebungen hinweg ermöglicht. \footnote{Unity-Dokumentation, \cite{Prefabs}}\footnote{Unity-Dokumentation, \cite{Prefabs2}}

Prefabs bieten zahlreiche Vorteile für die Gestaltung und Entwicklung von Projekten. Ihr Hauptvorteil liegt in ihrer
Fähigkeit, eine effiziente und konsistente Gestaltung von Projekten zu ermöglichen. Sie erleichtern die Wiederverwendung
von Designelementen und tragen so zur Effizienz und Konsistenz des Designprozesses bei. \footnote{Unity-Dokumentation, \cite{Prefabs}}\footnote{Unity-Dokumentation, \cite{Prefabs2}}

In komplexen Projekten, die eine Vielzahl von Objekten wie Charaktere, Umgebungen und visuelle Effekte beinhalten, erweisen
sich Prefabs als äußerst nützlich. Sie dienen als wiederverwendbare Vorlagen, die es ermöglichen, einmal erstellte Elemente
zu speichern und in unterschiedlichen Szenarien wiederzuverwenden. Dies eliminiert die Notwendigkeit, jedes Mal, wenn eine
neue Szene erstellt werden muss, von Grund auf neu zu beginnen. Darüber hinaus trägt die Verwendung von Prefabs dazu bei,
eine konsistente Designästhetik über das gesamte Projekt hinweg zu gewährleisten. \footnote{Unity-Dokumentation, \cite{Prefabs}}\footnote{Unity-Dokumentation, \cite{Prefabs2}}
...
Dieser Abschnitt erläutert, wie die Webseite gestartet und genutzt wird. Der gesamte Code der Webseite ist in einer Datei
namens \textit{Level1-FrontEnd/index.html} enthalten. Im Folgenden wird erklärt, wie diese Datei ausgeführt wird, um die
Webseite im lokalen Netzwerk bereitzustellen.

\subsubsection{Webseite starten}

Um die Webseite zu starten, benötigen Sie einen lokalen Server. Wir haben uns für die Verwendung von \textit{http-server}
entschieden, aber die Wahl des Servers ist nicht relevant, solange die Webseite im lokalen Netzwerk erreichbar ist. Zur
Installation von \textit{http-server} auf einem Windows-Gerät verwenden Sie den Befehl \textit{npm install http-server -g}
in der Kommandozeile.

Nach der Installation des Servers kann die Webseite im Ordner, in dem sich die \texttt{index.html}-Datei befindet, mit dem
Befehl \textit{http-server} und den Flags \textit{-c -1 --cors} gestartet werden.

Die Flags haben folgende Bedeutungen:

\texttt{-c-1}: Das \textit{-c} Flag legt die Cache-Zeit der Webseite fest. Durch Kombination mit -1 wird das Caching
deaktiviert.
\texttt{--cors}: Die Option \textit{--cors} aktiviert CORS (Cross-Origin Resource Sharing) auf dem HTTP-Server. Weitere
...
Der Server wird standardmäßig auf dem Port 8080 gestartet. Falls ein anderer Port gewünscht ist, kann dieser mit dem Flag
\textit{-p} und der gewünschten Portnummer geändert werden.

Nach dem Starten der Webseite können Sie diese im Browser Ihrer Wahl über die IP-Adresse des Geräts oder über \textit{localhost}
aufrufen. Die IP-Adresse des Geräts finden Sie mit dem Befehl \textit{ipconfig} in der Kommandozeile.

Nach dem erfolgreichen Starten der Webseite müssen Sie zunächst die IP-Adresse der HoloLens eingeben. Dies geschieht über
das Zahnrad in der oberen linken Ecke der Webseite. Die IP-Adresse wird in das Eingabefeld eingetragen und mit dem Button
\textit{Verbinden} bestätigt. Erst nachdem die IP-Adresse eingetragen wurde, können Nachrichten gesendet werden.

\begin{figure}[H]
    \centering
    \fbox{\includegraphics[width=1\textwidth]{images/WebseiteSettings.png}}
    \caption{Fesnter der Webseite Einstellungen}
...
sich das Gerät befindet. Dies ist erforderlich, um die Nachrichten richtig in der virtuellen Umgebung zu simulieren. Dazu
wählen Sie in der Combobox entweder \textit{Rechtes Gerät} oder \textit{Linkes Gerät}, je nach Position des Trägers der Brille.

Nach der erfolgreichen Konfiguration der Webseite können Sie Nachrichten versenden.

Falls beim Senden der Anfragen ein CORS-Fehler auftritt, kann das Problem auf folgende Weise gelöst werden:

\begin{itemize}
    \item \textbf{Deaktivieren im Browser:} Die Deaktivierung von CORS im Browser würde das Problem beheben, birgt jedoch
    gewisse Risiken. CORS schützt den Benutzer vor schädlichen Webseiten, daher wird davon abgeraten. Die Deaktivierung
    erfolgt je nach Browser und ist nicht einfach zu finden. Stattdessen sollten die nachfolgende Lösung in Betracht gezogen werden.
    \item \textbf{Verwendung von Plugins:} Es gibt Plugins, die CORS-Header von der Clientseite hinzufügen. Wir haben das
    Plugin \textit{moesif-origin-cors-change} getestet, das nur für den Chrome-Browser von Google verfügbar ist. Nach der
    Installation kann das Plugin bei Bedarf aktiviert/deaktiviert werden.
\end{itemize}

Es ist wichtig zu beachten, dass beide Lösungsvorschläge gewisse Sicherheitsrisiken bergen und nur im lokalen Netzwerk
getestet wurden. Nach der Verwendung unseres Programms sollten beide Konfigurationen wieder rückgängig gemacht werden.

\subsubsection{\label{sec:cors}CORS}
CORS (Cross-Origin Resource Sharing) ist ein Sicherheitsmechanismus in Webbrowsern, der kontrolliert, wie Ressourcen von
einer Webseite aufgerufen werden können, die sich auf einer anderen Domain befinden. Standardmäßig erlauben Browser nur
Anfragen an Ressourcen derselben Domain aus Sicherheitsgründen. CORS ermöglicht jedoch den Zugriff auf Ressourcen von anderen
Domains, sofern der Server dies explizit zulässt.

Um dies zu ermöglichen, sendet der Browser bei Anfragen an andere Domains sogenannte CORS-Anfragen an den Server, um zu
überprüfen, ob der Zugriff erlaubt ist. Der Server antwortet dann mit spezifischen CORS-Headern, die angeben, ob die Anfrage
erlaubt ist und von welchen Ursprüngen aus der Zugriff gestattet ist. Da unsere Kommunikation nur im lokalen Netzwerk läuft,
ist es für uns nicht von großer Bedeutung, diese Sicherheitsmaßnahmen zu ergreifen. Dennoch müssen wir folgende Header bei
allen Antworten, die von der Brille zurückgeschickt werden, einbauen:
...
Diese drei Zeilen fügen CORS-Header zur HTTP-Antwort hinzu, wodurch Cross-Origin-Anfragen für Ressourcen von diesem Server
ermöglicht werden. Sie definieren auch genau, welche HTTP-Methoden und Header in diesen Anfragen erlaubt sind.

Diese Header sind nicht dazu da, Sicherheitsmaßnahmen zu treffen, sondern um sie zu umgehen. Die HoloLens ist so konfiguriert,
dass sie nur Anfragen aus dem lokalen Netzwerk annimmt.
...
\begin{itemize}
    ...
    \item \textbf{Übersetzung der Positionen:} Die Positionen der erkannten QR-Codes werden in die Weltkoordinaten der HoloLens übersetzt, um sie in der virtuellen Welt zu verankern. Dazu wird ein Spatial Graph Node erstellt und die Position des QR-Codes an diesen gebunden.
    \item \textbf{Zuordnung des QR-Codes zum Objekt:} Nach der Erkennung wird geprüft, ob der QR-Code bereits in der Anwendung registriert ist. Ist dies der Fall, wird er einem entsprechenden Objekt in der virtuellen Umgebung zugeordnet.
\end{itemize}
...
\begin{itemize}
\item \textbf{Canvas:} Der Canvas enthält drei Labels: "Name", "Gewicht" und "Wert", welche die für den Benutzer relevanten Eigenschaften anzeigen.
...
\begin{lstlisting}[style=csharp label=code:QR-Prefab instanzierung]
void Start()
{
    //... weiterer Code
...
Nach der Instanzierung eines QR-Prefabs wird der folgende Codeabschnitt (\ref{code:QR-Prefab instanzierung}) ausgeführt. Das Skript \textit{QRCode.cs}, in dem sich dieser Abschnitt befindet, erhält bei der Instanzierung lediglich den Inhalt des gescannten QR-Codes übermittelt, nämlich die ID. Anhand dieser ID werden aus einer Liste möglicher Objekte die weiteren erforderlichen Informationen entnommen und innerhalb der Klasse gespeichert. Zudem wird das entsprechende 3D-Modell in die Szene eingefügt, und die Labels werden mit den richtigen Werten (Name, Gewicht, Wert) befüllt. Diese Aktionen werden bei jeder Instanzierung nur einmal ausgeführt. Im Gegensatz dazu wird die folgende Funktion (\ref{code:Aktuallisieren der Position}) bei jedem Frame ausgeführt und befindet sich daher in der \textit{Update()} Funktion.

\begin{lstlisting}[style=csharp label=code:Aktuallisieren der Position]
...
\item Die Position des QR-Codes \textit{item.qrData.position} wird auf \textit{(PhysicalSize / 2.0f, PhysicalSize / 2.0f, 0.0f)} gesetzt. Das heißt, Position des QR-Codes wird relativ zu seiner eigenen Größe festgelegt, wodurch er sich in seinem eigenen Koordinatensystem zentriert.
...
\item Die Position des 3D-Modell, das mit dem QR-Code verbunden ist wird auf die Position des QR-Codes gesetzt. Schließlich wird \textit{lastTimeStamp} auf die aktuelle Zeitmarke des QR-Codes gesetzt, um festzuhalten, wann der QR-Code zuletzt erkannt wurde.

\end{itemize}

Diese Prozesse gewährleisten nicht nur eine korrekte Darstellung der QR-Codes in der virtuellen Welt, sondern ermöglichen auch deren Interaktivität.

\subsubsection{Spatial Graph Node}
...
Das multiple Knapsack-Problem \textit{erweitert} das klassische Knapsack-Problem, indem es \textit{mehrere} Rucksäcke oder Behälter
...
gepackt wird oder \textit{überhaupt nicht}. Es gibt keine Möglichkeit, einen Gegenstand \textit{teilweise} in
...
Sie akzeptiert drei Parameter: ein Array von Gewichten (\( \texttt{weights} \)), ein Array von Werten (\( \texttt{values} \))
und die Kapazität des Rucksacks (\( \texttt{capacity} \)).

Zuerst wird die Länge des Gewichtsarrays \( \texttt{n} \) berechnet, um die Anzahl der verfügbaren Gegenstände zu bestimmen.
Dann wird eine Tabelle \( \texttt{Tabelle} \) mit \( \texttt{n+1} \) Zeilen und \( \texttt{capacity+1} \) Spalten initialisiert.
...
END FUNCTION
\end{lstlisting}\\
Die Funktion \texttt{knapsackGreedy()} nimmt wie der dynamische Ansatz drei Parameter an: ein Array von Gewichten (\( \texttt{weights} \)),
ein Array von Werten (\( \texttt{values} \)) und die Kapazität des Rucksacks (\( \texttt{capacity} \)). Die Funktionsweise
...
Nach einer gründlichen Analyse der Aufgabenstellung des Projekts wurde die endgültige Entscheidung für die Variante und den
Typ getroffen. Dabei wurde festgestellt, dass das klassische Knapsack-Problem mit einer Kombination der beiden Problem-Typen
0/1-Knapsack-Problem und Begrenztes-Knapsack-Problem die geeignete Wahl ist. Die Entscheidung für den Typ 0/1 des Knapsack-Problems
wurde getroffen, da die Aufgabenstellung des Anwendungsszenarios vorsieht, dass der Benutzer Gegenstände in einem Inventar
nur vollständig hinzufügen oder gar nicht hinzufügen kann. Und die Kombination mit dem Begrenzten Knapsack-Problem deswegen,
weil in dieser Anwendung eine Begrenzung an Gegenständen, die dem Inventar hinzugefügt werden können, gesetzt ist.


\subsubsection{Der Knapsack Solver}
Das Unity Game Objekt \textit{Knapsack Solver}, wie in Abbildung \ref{fig:Knapsack_Editor} dargestellt, implementiert die
Lösung des Knapsack-Problems und berechnet gleichzeitig das zusammengestellte Inventar des Benutzers. Es stellt Feedback
über diese Berechnungen bereit. Dieses Game Objekt mit der angehängten Skript-Komponente \textit{Knapsack Solver} implementiert
die Klasse \textit{KnapsackSolver}, welche die Logik zur Berechnung des maximal erreichbaren Werts einer optimalen Lösung
sowie den Wert des selbst zusammengestellten Inventars steuert.
...
\begin{itemize}
    ...
    \item \textbf{Own Mesh:} Referenz auf das \textit{OwnValueMesh} aus dem Inventar Prefab

    \item \textbf{Max Mesh:} Referenz auf das \textit{MaxValueMesh} aus dem Inventar Prefab

    \item \textbf{Info Mesh:} Referenz auf das \textit{infoMesh} aus dem Inventar Prefab
...
Einschränkung resultiert aus der Tatsache, dass das vorliegende Inventar nur neun verfügbare Zellen hat, in denen ein
...
Während des Algorithmus wird zusätzlich ein Array aus booleschen Werten erstellt, um die ausgewählten Gegenstände für den
maximalen Wert zu markieren. Dieses Array spielt eine wichtige Rolle im zweiten Abschnitt der Funktion, um diese Gegenstände
zurückzuverfolgen und die perfekte Lösung zusammenzustellen.
...
\]
\\
Und die \textit{Kapazität} des Rucksacks liegt bei 5. Aufgrund dieser Angaben wird die \texttt{selected} Matrix nach
...
\subsubsection{Berechnung des zusammengestellten Inventars}
Um dem Benutzer dementsprechend Feedback zu geben und dadurch zu veranschaulichen, welchen Wert das selbst zusammengestellte
Inventar im Vergleich mit dem maximal erreichbaren Wert hat, wird hier eine zusätzliche Berechnung durchgeführt, um diesen
Wert zu errechnen. Diese Berechnung wird in der Funktion \texttt{KnapsackInventoryValue()} durchgeführt.
...
\end{lstlisting}
Diese Methode durchläuft jedes Element im Inventar und überprüft, ob es mit einem Gegenstand aus einer internen Datenstruktur
übereinstimmt. Falls ja, wird der Wert dieses Gegenstands zum Gesamtwert addiert. Am Ende wird der berechnete Gesamtwert zurückgegeben.
