...
In dem Absatz \ref{sec:FrontendWebseite} wurde der Designprozess der Webseite beschrieben, hier ist eine erweiterung zu dem Thema und enthält die beschreibung der Funktionalitäten und die Umsetzung davon.

Die Webseite ist die Schnittstelle für die Kommunikation zwischen den 2 Laptops die wir für das Szenario brauchen. Diese Kommunikation geschieht über die Hololens, sie ist sozusagen der "Middleman" welches die übertragung von Daten ermöglicht.

\subsubsection{Protokoll zur Datenübertragung}

% Übertragungsprotokoll

In der Abbildung \ref{fig:protokoll} ist der Ablauf aller möglichen Szenarien wie solch eine Datenübertragung zustande kommen kann. Folgend werden die einzelnen Schritte und wozu sie dienen beschrieben.

\begin{itemize}
    \item \textbf{Start des Servers: } Auf der Holoens befindet sich ein Game-Objekt \textit{httpListener} mit einem \textit{RequestManager.cs} Skript. Beim Laden des ersten Szenarios wird ein http-listener in einem Thread gestartet. Erst wenn dieser läuft kann auf Anfragen der Webseite reagiert werden.
    \item \textbf{Verbindungsaufbau: } Bevor ein Nachrichtenpaket versendet werden kann muss die IP-Addresse der HoloLens registriert werden. Um zu testen ob die richtige IP-Addresse eingegeben wurde oder der Server läuft wird ein \texttt{http get request} an den Endpoint \textit{connect} gesendet.
    \item \textbf{Empfangen von Nachrichten: } Auf der Brille wird ein stand an aktuellen Nachrichten festgehalten. Die Webseite fragt alle fünf Sekunden, mit einem \textit{http get request} an den Endpoint \textit{get-messages}, diese Liste an Nachrichten ab.
    \item \textbf{Versenden von Nachrichten: } Um eine Nachricht von einem Laptop auf den anderen zu bekommen muss ein \textit{http post request} an den Endpoint \textit{message} mit dem Benutzernamen und Nachricht, als Payload, gesendet werden.
\end{itemize}
Das senden von requests mag kompliziert wirken aber wie in dem Abschnitt \ref{sec:Frontend} zu sehen ist wird das alles mithilfe einer benutzerfrundlichen UI erreicht.

Es ist festzuhalten das die beiden Laptops NIE direkt untereinander kommunizieren sondern alle Nachrichten über den Server, die HoloLens, laufen.

\subsubsection{Anfragen bearbeiten}
Das bearbeiten von Anfragen ist in dem \textit{RequestManager.cs} Skript erledigt. Der listener wird in einem eigenen Thread gestartet, da das warten auf Nachrichten den Thread auf dem der listener läuft blockiert. Nach dem erhalt von einem request wird ein neues \textit{HttpListenerContext} Objekt erstellt und die Verarbeitung des requests wird in dem Thread durchgeführt, der diesen Kontext bearbeitet.
Wenn eine http Anfrage erhalten wird, ist der erste Schritt zu prüfen welche Art an Anfrage es ist. Ein POST und ein GET werden unterschiedlich verarbeitet, bei verwendung jeglicher anderen http Methode wird ein \textit{Method Not Allowed} Fehler zurück gesendet. Bei der Wahl des Endpoint passiert das selbe, jegliche nicht unterstützte Anfrage auf einen Endpoint, wird mit einem \textit{Not Found} Fehler geantwortet.

Es werden in unserem Fall nur 3 Endpunkte unterstützt:


\begin{itemize}
    \item \textbf{Testen der Verbindung auf /connect}
    \item \textbf{Erhalten von Nachrichten auf /get-messages}
    \item \textbf{Senden einer Nachricht auf /message}
\end{itemize}

Um and den Server Erfolgreich eine Anfrage zu senden muss er an folgende Adresse gehen:
\texttt{http://{IpAddresseDerBirlle}:{Port}/{endpoint}}

Die IP-Addresse der Brille wird manuell auf der Webseite eingegeben.\\
Der listener läuft standartmäßig auf dem port 9090. \\
Die erlaubten endpoints sind Fallbasierend oben gelistet.
