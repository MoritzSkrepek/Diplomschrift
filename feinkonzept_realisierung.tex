% Define colors
\definecolor{keywordcolor}{RGB}{0, 0, 255} % Blue
\definecolor{Classcolor}{RGB}{43, 145, 175} % Blue-Green
\definecolor{Funcolor}{RGB}{128, 0, 128} % Purple (functions)
\definecolor{Logiccolor}{RGB}{163, 21, 21} % Red
\definecolor{commentcolor}{RGB}{0, 128, 0} % Green (comments)
\definecolor{stringcolor}{RGB}{0, 128, 0} % Red
...
\lstdefinelanguage{CS}{
    language=C,
    morekeywords=
        {using, public, private, class, yield, return, new, if, else, out, try, catch, for, while, throw, foreach, in, using, Vector2, Pose},
    morekeywords=[2]
        {ARRaycastManager, ARPlaneManager, GameObject, InventoryController, Vector3, ARPlane, Ray, RaycastHit, GetComponent, MeshRenderer, Quaternion, Transform},
    morekeywords=[3]
        {Start, Update, Awake, DelayedStart, IsPointerOverPlane, GetCurrentPlaneUnderGaze, SetPlaneColor, PlaceObjectOnDesk, ScreenPointToRay, SetActive, Euler, SetInventoryObject, Raycast, ChangeScene, MoveToTarget, stopFunctionCoroutine, stopAllCurrentCoroutine, DelayedStart, takingPicture, LoadingCoroutine, Execute, SearchForRedPixels, editAndSendPackage, GetTextMeshProFromChild, SetInventory, SetUp, KnapsackMaxValue_NoItems_ReturnsZero, KnapsackPerformanceReportItemRange, KnapsackPerformanceReportRangeIteration, setLabels, KnapsackMaxValue, KnapsackInventoryValue, GridUpdate, SendMsg, ReceiveMsg, KnapsackMaxValue_SingleItem_ReturnsExpectedValue, KnapsackMaxValue_MultipleItems_ReturnsExpectedValue, UpdateInventoryBounds, GetBounds, ExtendedBounds, setNewPosition, fillInventory, editTextureV2, DebugCameraResolution, isNumberKey, validateAndSendMessage, UpdatePropertiesDisplay,
    // Functions from MRTK3 and other Unity AR development frameworks
    MixedRealityToolkit, MixedRealityPlayspace, MixedRealityInputManager, MixedRealityToolkitVisualProfiler, HandTrackingInputSimulationProfile, BoundarySystem, ManipulationHandler, ObjectManipulator, NearInteractionTouchable, NearInteractionGrabbable, Follow, Interactable, ArticulatedHandDefinition, ArticulatedHandPose, HandJointUtils},
    morekeywords=[4]
...
\lstset{
    ...
    commentstyle=\color{commentcolor}, % Green comments
    ...
    rulecolor=\color{black}, % Black frame rule
    framerule=0.5pt, % Thin frame rule
% You can adjust more settings to mimic Visual Studio 2022
}
...
\section{Entwicklungsumgebungen}\marginpar{\small\(\rightarrow\) {\tiny SKREPEK}}
...
Diese Methoden folgen einem genau definierten Ablauf, der den Zeitpunkt und die Reihenfolge ihres Aufrufs, ihre Ausführung
und ihre Abwicklung regelt. Entwickler haben somit eine klare Kontrolle über den Fluss ihrer Anwendungslogik und können
präzise Aktionen zu bestimmten Zeitpunkten im Lebenszyklus der Anwendung ausführen. Im folgenden Abschnitt wird dieser
Ablauf durch ein Diagramm veranschaulicht.
...
\footnotetext{Unity-Dokumentation \cite{Order of execution for event functions}}
...
Unity-Szenen (Siehe Abschnitt \ref{sec:jonasSzenario} oder \ref{sec:Scene2} für Umsetzung) bilden das grundlegende Gerüst für
...
Im Kontext des Anwendungsszenarios \textit{Nachrichtenaustausch} spielen beide Manager eine wichtige Rolle. Der ARPlaneManager
...
\subsubsection{\label{sec:prefabs}Unity Prefabs}\marginpar{\small\(\rightarrow\) {\tiny HAYLAZ}}
...
\subsection{Deployment der Anwendung}\marginpar{\small\(\rightarrow\) {\tiny HAYLAZ}}
...
\begin{itemize}
    ...
    \item \textbf{Authentifizierung:} Der Computer, der sich mit der HoloLens verbinden und die Anwendung deployen möchte, muss von der Brille authentifiziert werden. Dies gewährleistet, dass nur autorisierte Geräte Zugriff auf die Brille haben und die Übertragung sicher
    erfolgt. Wie die Authentifizierung durchgeführt wird, wird in Abschnitt \ref{sec:erstmal} erläutert.
\end{itemize}
...
% https://learn.microsoft.com/en-us/windows/apps/design/devices/designing-for-mr#overview

Es gibt verschiedene Build-Einstellungen, wie in Abbildung \ref{fig:build-settings} dargestellt. Besonders relevant sind dabei die Einstellungen für die \textit{Architektur} und die \textit{Build-Konfiguration}. Die
...
% https://learn.microsoft.com/en-us/training/modules/learn-mrtk-tutorials/1-7-exercise-hand-interaction-with-objectmanipulator
% https://learn.microsoft.com/de-de/windows/mixed-reality/develop/advanced-concepts/using-visual-studio?tabs=hl2
...
\subsubsection{\label{sec:erstmal}Erstmaliges Deployment}
...
\section{Objektdesign}\marginpar{\small\(\rightarrow\) {\tiny LAMPEL}}
...
In Abbildung \ref{fig:shadereditor} ist zu erkennen, wie die Image-Textur-Node den Farbwert an das Material weitergibt. Als Beispiel wurde hier der Taschenrechner verwendet, der im Laufe des Kapitels noch näher erläutert wird. Die Abbildung zeigt die verschiedenen Konfigurationsmöglichkeiten, auf die bereits im Abschnitt \ref{sec:bildtextures} eingegangen wurde.
...
Modifier sind ein wichtiger Bestandteil der 3D-Modellierung. Sie bieten eine flexible und leistungsstarke Möglichkeit, Objekte zu bearbeiten und zu verbessern, ohne die Integrität des Modells zu beeinträchtigen. Die Verwendung von Modifiern kann den Modellierungsprozess rationalisieren und die Kreativität des Modellierenden fördern.
...
Der Mirror Modifier spiegelt die Geometrie eines Objekts entlang einer oder mehrerer Achsen. Dadurch können symmetrische Modelle erstellt werden, indem nur eine Hälfte modelliert werden muss. Der Modifier erzeugt automatisch die gespiegelte Seite des Objekts, wodurch Zeit und Aufwand gespart wird. Der Mirror Modifier wird häufig bei der Modellierung von symmetrischen Objekten wie Menschen, Fahrzeugen und Gebäuden eingesetzt. Im Projekt wurde er beispielsweise für das Modell des Taschenrechners eingesetzt, um die Basisform gleichmäßig auf beiden Seiten zu formen. \footnote{Blender \cite {Mirror Modifier}}
...
Zu Beginn des Projekts musste eine wichtige Entscheidung getroffen werden, nämlich wie viele Objekte modelliert werden sollen. Es war wichtig, eine ausgewogene Anzahl zu wählen, die die Applikation nicht überladen, aber dennoch eine Herausforderung für die Benutzer darstellen soll. Nach eingehenden Recherchen und internen Abstimmungen einigte sich das Team auf 11 Gegenstände. Diese sollten alltägliche Gegenstände eines Schülers der HTBLuVA darstellen und den Anwendern einen Einblick in den Schulalltag bieten.
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.5\textwidth, angle=90]{images/basis.png}
    \caption{Taschenrechnerbasis in Blender}
    \label{fig:Basis}
\end{figure}
...
Die Abbildung zeigt den \textit{UV-Editing-Editor} in Blender. Mit diesem Editor können die \textit{UV-Maps} der 3D-Modelle bearbeitet werden, indem die 3D-Geometrie auf eine zweidimensionale Ebene projiziert wird, um die Positionierung der Texturen zu steuern. \footnote{Blender Dokumentation \cite{UV-Editing Editor}}
...
\section{Hauptmenü}
Im folgenden Abschnitt werden die Entwicklungsphasen beim Entwurf des Menüs erläutert. Es wird erklärt, welche Probleme dabei aufgetreten sind und worauf bei der Benutzeroberfläche (UI) und der Benutzererfahrung (UX) geachtet werden muss.
...
\begin{figure}[h]
    ...
    \caption{Darstellung des finalen Menues innerhalb der Anwendung}
...
In der Abbildung \ref{fig:hierarchiemenuunity} ist die Hierarchie des Hauptmenüs im Unity Editor zu sehen. Dem Prefab \textit{menu} sind dabei 5 einzelne Objekte zugewiesen. Die Prefabs \textit{startbuttonclickable\_lv1/2} sind die in Unity erstellten unsichtbaren Buttons, die dafür zuständig sind den vormodellierten Buttons eine Funktion zu geben. Darunter zu sehen sind die 3 verschiedenen Assets, welche aus Blender importiert wurden. Diese bestehen jeweils aus den verschiedenen Einzelteilen, welche in Unity als Gameobjects dargestellt werden. Die \textit{BezierCurve} ist eine spezielle Art von Kurve welche in Blender für die leichte Wölbung der Tafeln genutzt wurde. \footnote{Blender \cite{Bezier Curve}}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.6\textwidth]{images/backbutton.png}
    \caption{Darstellung des finalen Zurückbuttons innerhalb der Anwendung}
...
\section{\label{sec:jonasSzenario}Nachrichtenaustausch Anwendungsszenario} \marginpar{\small\(\rightarrow\) {\tiny Schoditsch}}
...
\subsection{Aufbau von Nachrichtenaustausch Anwendungsszenario}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\textwidth]{images/Level1Hierarchy.png}
...
\end{itemize}
\subsubsection{Aufbau in der realen welt}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{images/AufbauAnzeige.png}
...
\subsubsection*{Beispiel Code Coroutine}
...
\subsubsection{\label{sec:IEnumerator}IEnumerator Klasse}
...
\subsubsection{\label{sec:Canvas}Unity Canvas}
...
kann auch als weiteres child ein \textit{Image} besitzen. Das Canvas wird als Rechteck dargestellt, was es erleichtert, die genaue Position des Objekts zu bestimmen. Ein Canvas verfügt auch über verschiedene Rendermodi:
\begin{itemize}
...
\subsection{Ladesymbol}\label{sec:ladeSymbol}
...
\begin{figure}[H]
    ...
    \begin{minipage}[b]{0.6\textwidth}
        ...
        \caption{Hierarchie Lade Kreis im Unity Editor}
    \end{minipage}
    \hspace{1cm}
    ...
    \end{minipage}
\end{figure}
\begin{lstlisting}[style=csharp, caption={Code zum ausfuehren des lade Symbols}, label=code:LoadingCoroutine]
    ...
    int textureWidth = textureToEdit.width;
            ...
            for (int y = 0; y < textureToEdit.height; y++) {
                for (int x = 0; x < textureWidth; x++) {
                    ...
                    if (pixels[index].r > 0.55f && pixels[index].g < 0.5f &&
                        pixels[index].b < 0.5f) {
                        yCoordinates.Add(y);
                        xCoordinates.Add(x);
                    }
...
struct PixelJob : IJobParallelFor {
    ...
    public void Execute(int batchIndex) {
        ...
        int startPIndex = batchIndex * batchSize;
        // Berechnet den Endindex fuer den aktuellen Batch
        int endPIndex = Mathf.Min((batchIndex + 1) * batchSize, pixels.Length);
        for (int pIndex = startPIndex; pIndex < endPIndex; pIndex++) {
            // Berechnet die x- und y-Koordinaten des aktuellen Pixels.
            int x = pIndex % textureWidth;
            int y = pIndex / textureWidth;
            // Ueberprueft, ob der Pixel rot genug ist.
            if (pixels[pIndex].r > 0.7f && pixels[pIndex].g < 0.5f &&
                pixels[pIndex].b < 0.5f) {
                pixels[pIndex] = Color.red;
                redPixelPositions[pIndex] = new Vector2Int(x, y);
            } else {
                pixels[pIndex] = Color.black;
            }
        }
    }}
\end{lstlisting}
\begin{itemize}
    \item \textbf{NativeArray:} bieten verbesserte Leistung im Vergleich zu standardmäßigen Arrays, da diese direkt auf native Speicherbereiche zugreifen können, was besonders wichtig ist, wenn große Datenmengen verarbeitet werden müssen und die Ausführung parallelisiert werden sollen.
...
List<Vector2> SearchForRedPixels(int startX, int startY) {
    // momentan gefundene Linie von roten Pixeln
    List<Vector2> currentLine = new List<Vector2>();
    bool hasFound = false;
    // mommentane x/y Positionen der roten Pixel
    int foundX = startX, foundY = startY;
    // ueberprueft ob die hoehe nicht uebertroffen wird
    int heightY = redPixels.GetLength(1);
    // ueberprueft ob die breite nicht uebertroffen wird
    int widthX = redPixels.GetLength(0);

    do {
        hasFound = false;
        redPixels[foundX, foundY] = false;
        currentLine.Add(new Vector2(foundX, foundY));
        // ueberprueft die naechsten 25 pixel rechts ob ein roter pixel vorhanden ist
        for (int i = 1; i < 25 && foundX + i < widthX; i++) {
        ...
        // ueberprueft die naechsten 10 pixel unten ob ein roter pixel vorhanden ist
        ...
        // ueberprueft die naechsten 10 pixel darueber ob ein roter pixel vorhanden ist
...
\begin{itemize}
    ...
    \item \textbf{widthX}: Die Breite des Arrays \textit{redPixels}.
\end{itemize}


Die Funktion sucht nach roten Pixeln, indem sie von einer Startposition ausgehend Pixel in der Nähe überprüft. Dabei wird eine Liste der gefundenen Pixel Positionen erstellt. Die Schleife wird, solange durchlaufen, bis kein roter Pixel mehr gefunden wird. Innerhalb der Schleife wird zuerst das aktuelle Pixel als geprüft markiert und seine Position zur Liste hinzugefügt. Dann wird nach roten Pixeln in der Nähe gesucht. Falls keiner gefunden wird, wird die Schleife beendet. Am Ende wird die Liste der gefundenen Pixel zurückgegeben.


\subsection{\label{sec:jonasNachrichtenSimulation}Erstellen und Bewegung des Paketes}
Um das Paket über das Kabel gleiten zu lassen, muss zuerst der Abstand gemessen werden. Sobald dieser gemessen wurde, sind alle wichtigen Punkte berechnet, über die das Paket gleiten soll. Dann muss das Objekt, welches das Paket darstellt, initialisiert und am Anfang des Kabels positioniert werden. Danach wird das Paket über die Punkte bewegt.

\subsubsection{\label{sec:EntfernungMessen}Entfernung messen}
...
\begin{itemize}
    \item \texttt{redPixel.x} und \texttt{redPixel.y} sind die Positionen, welche in \texttt{screenCoordinates} umgerechnet werden sollen.
    \item \texttt{Camera.main.pixelWidth} und \texttt{Camera.main.pixelHeight} sind die Breite und Höhe des Bildschirms, auf dem die Kameraansicht dargestellt wird.
    \item Unter \texttt{image.width} und \texttt{image.height} versteht man die gesamte Breite und Höhe des verarbeiteten Bildes vom roten Kabel in Pixels.
\end{itemize}
Sobald dann die \texttt{screenCoordinates} berechnet wurden, kann der Raycast auf die Position im Bildschirm aufgerufen werden. Dafür ist dieser Code teils von \texttt{positionVirtualObject} zuständig:
\begin{lstlisting}[style=csharp, caption={Raycast schiessen}, label=code:createRaycast]
...
if (raycastManager.Raycast(screenCoordinates, hits, TrackableType.Planes)) {
    ...
    for (int i = 0; i < hits.Count; i++) {
        ...
        if (hits[i].hitType != TrackableType.PlaneWithinInfinity) {
            if (hits[i] != null) {
                // Die Position des Treffers um 0.05 Einheiten nach oben verschieben
                // (fuer eine bessere Anzeige) und zur Liste der
                // Kabelpositionen hinzufuegen.
                cablePositions.Add(new Vector3(hits[i].pose.position.x,
                                   hits[i].pose.position.y + 0.05f,
                                   hits[i].pose.position.z));
                // Schleife bei gueltigem Treffer verlassen
                break;
            }
        }
    }
...
public void editAndSendPackage(string username, string message) {
    if (isAtEnd) {
        // zuruecksetzen der Flags und Zaehler
        isAtEnd = false;
        moveToCounter = 1;
        moveFromCounter = 0;
    }
    ...
    MainThreadDispatcher.Instance().Enqueue(() => {
        if (isObjectInstantiated == false) {
            instantiatedObject = Instantiate(virtualObject,
                                             cablePositions[0],
                                             Quaternion.identity);
        ...
        // Ein Textobjekt fuer die Informationen erstellen und bearbeiten
        EditTextOfInformationObject editTextOfInformationObject =
                                    new EditTextOfInformationObject(infoTextS);
...
void Update() {
    if (shouldMove) {
        // Ueberpruefen, ob Index des Pakets kleiner als Anzahl der Kabelpositionen
        if (moveToCounter < cablePositions.Count) {
            // Das Paket in die richtung der naechsten Position bewegen
            instantiatedObject.transform.Translate((cablePositions[moveToCounter] -
                                                    cablePositions[moveFromCounter]) *
                                                    0.0075f);

            // Ueberpruefen, ob das Objekt nahe genug an der Zielposition ist
            if (Vector3.Distance(instantiatedObject.transform.position,
                                 cablePositions[moveToCounter]) < 0.00375f) {
                ...
                if (Vector3.Distance(instantiatedObject.transform.position,
                                     cablePositions[cablePositions.Count - 1]) <
                                     0.00375f) {
                    isAtEnd = true;
                }
            }
...
Der Code prüft zuerst, ob das Objekt beweget werden soll. Dann wird überprüft, ob das Objekt bereits am Ende angelangt ist, indem der \textit{moveToCounter} mit der Anzahl der Positionen in \textit{cablePositions} verglichen wird.
...
public class EditTextOfInformationObject : MonoBehaviour {
    // Referenz auf das GameObject, welches das TextMeshPro-Objekt enthaelt
    public GameObject parentGameObject;
    // Referenz auf das TextMeshPro-Objekt, dass bearbeitet werden soll
    TextMeshPro textMeshPro;

    public void GetTextMeshProFromChild() {
        if (parentGameObject != null) {
            // Das Childobjekt mit dem Namen "InformationTextS" im
            // ElternGameObject finden
            GameObject childGameObject = parentGameObject.transform
                                         .Find("InformationTextS").GameObject;
...
void debugRaycast(ARRaycastHit hit) {
    ...
    GameObject instantiatedObject = Instantiate(virtualObject,
                                                hit.pose.position,
                                                Quaternion.identity);
}
\end{lstlisting}
...
\subsection{\label{sec:Website}Starten und Nutzen der Webseite}\marginpar{\small\(\rightarrow\) {\tiny HAYLAZ}}
...
Die gesamte Anwendung ist bewusst einfach und minimalistisch gehalten. Folglich befindet sich der gesamte Code der Webseite in einer einzigen Datei, \textit{index.html}. Im Folgenden wird erläutert, wie diese Datei ausgeführt wird, um die Webseite im lokalen Netzwerk bereitzustellen.
...
Nach der Installation des Servers kann die Webseite im Ordner, in dem sich die \textit{index.html}-Datei befindet (standardmäßig im \textit{Level1-FrontEnd} Ordner), mit dem Befehl \textit{http-server} und den Flags \textit{-c -1 --cors} gestartet werden.
...
In dem gegebenen Befehl wird index.html als Argument übergeben. Dies bedeutet, dass der http-server darauf eingestellt ist, die Datei \textit{index.html} zu liefern, wenn die Wurzel-URL des Servers aufgerufen wird. Der Server wird den Inhalt dieser Datei dem Client zurückgeben, der die Anfrage gestellt hat. Wie bereits erwähnt ist der gesamte Inhalt der Webseite in dieser Datei enthalten, da es sich um eine einfache \textiit{Single-Page-Applikation (SPA)} handelt. \footnote{Bloomreach \cite{SPA}}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{images/WebButton}
    \caption{Position des Konfigurationsbuttons auf der Webseite}
    \label{fig:webbut}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.55\textwidth]{images/WebseiteSettings}
    \caption{Konfigurationsfenster der Webseite}
...
% https://aws.amazon.com/de/what-is/cross-origin-resource-sharing/#:~:text=Cross%2DOrigin%20Resource%20Sharing%20(CORS,Ressourcen%20in%20einer%20anderen%20Domain.
...
\subsection{\label{sec:FrontendWebseite}Frontend der Webseite}\marginpar{\small\(\rightarrow\) {\tiny LAMPEL}}
...
\newpage
\subsection{\label{sec:webBackend}Backend der Webseite}\marginpar{\small\(\rightarrow\) {\tiny HAYLAZ}}
...
\begin{figure}[H]
    ...
    \begin{minipage}[b]{0.45\textwidth}
        \centering
        \includegraphics[scale=0.25]{images/serverStart.png}
    ...
    \begin{minipage}[b]{0.45\textwidth}
        \centering
        \includegraphics[scale=0.2]{images/laptopAufbau.png}
        \caption{Kommunikationsstruktur}
        \label{fig:struktur}
    \end{minipage}
\end{figure}

Abbildung \ref{fig:protokoll} zeigt den Ablauf aller möglichen Szenarien, wie eine solche Datenübertragung zustande kommen kann. Wie in Abbildung \ref{fig:struktur} zu erkennen ist, ist die HoloLens das Zentrum der Datenübertragung, über die die gesamte Kommunikation erfolgt.
...
gehen: \textit{http://{IP-Adresse der HoloLens}:{Port}/{Endpoint}}. Die IP-Adresse der HoloLens wird manuell auf der
...
\newpage
\section{Knapsack Problem Anwendungsszenario}\label{sec:Scene2} \marginpar{\small\(\rightarrow\) {\tiny SKREPEK}}
...
\subsection{Knapsack-Problem Szenrio Hirarchie} \marginpar{\small\(\rightarrow\) {\tiny SKREPEK}}
...
\begin{itemize}
    ...
    \item \textbf{InventoryPlacementController}: GameObject, welches das platzieren des Inventar-Objekts handhabt.
    \item \textbf{InventoryController}: GameObject, welches das erkennen von neuen Objekten innerhalb des Inventar-Objekts handhabt.
    \item \textbf{KnapsackSolver}: GameObject, welches den Knapsack Algorithmus implementiert und das eigene Inventar berechnet.
...
In der Abbildung ist ausßredem zu sehen, dass einige GameObject ausgegraut und nicht ausgegraut sind und, dass neben einigen GameObjects ein durchgestrichenes Auge zu sehen ist.
Wenn ein GameObject im Unity Editor ausgegraut ist bedeutet das, dass dieses GameObject und somit alle angehängiten Scripts von diesem GameObject deaktiviert sind.
Das bedeutet, dass dieses GameObject samt allen Scripts zu Szenenbeginn nicht aufgerufen und somit auch nicht ausgeführt werden. Nicht ausgegraute GameObject widerum sind
daher genau das Gegenteil. Das beudetet, dass das GameObject selbst samt allen angehängiten Scripts alle aktiviert sind und somit zu Szenenbeginn aufgerufen und ausgeführt werden.

Wenn neben einem GameObject das durchgestrichene Auge zu sehen ist bedeutet das nur, dass dieses GameObject im Unity Editor nicht zu sehen ist. Andererseits, wenn kein Zeichen
neben dem GameObject zu sehen ist, ist dieses Objekt im Unity Editor sichtbar. Dies dient dazu, dass falls in der Unity Szene viele GameObjects vorhanden sind, dass man
diejenige ausblendet die nicht im Editor sichtbar sein müssen wie zum Beispiel Tesh Meshes oder Lables.

\subsection{\label{sec:qrcodes}Nutzung von QR-Codes} \marginpar{\small\(\rightarrow\) {\tiny HAYLAZ}}
...
% https://learn.microsoft.com/de-de/windows/mixed-reality/develop/advanced-concepts/qr-code-tracking-overview#best-practices-for-qr-code-detection
...
\begin{lstlisting}[style=csharp, caption={QRItem Klasse}, label=code:update]
public class QRItem {
...
% https://learn.microsoft.com/en-us/previous-versions/msp-n-p/ee817670(v=pandp.10)?redirectedfrom=MSDN
% https://web.archive.org/web/20020424122433/http://home.earthlink.net/~huston2/dp/singleton.html
...
Durch den Einsatz der HoloLens kann dem Nutzer eine immersive visuelle Erfahrung in einer virtuellen Welt geboten werden. Um eine
...
\begin{itemize}
    ...
    \item \textbf{Übersetzung der Positionen}: Die Positionen der erkannten QR-Codes werden in die Weltkoordinaten (siehe \ref{sec:MoWeltpostionen}) der HoloLens übersetzt, um sie in der virtuellen Welt zu verankern. Dazu wird ein \textit{Spatial Graph Node}(siehe \ref{sec:SGN}) erstellt und die Position des QR-Codes an diesen gebunden.
...
%https://learn.microsoft.com/en-us/dotnet/api/microsoft.mixedreality.openxr.spatialgraphnode?view=mixedreality-openxr-plugin-1.10

\subsubsection{\label{sec:qrvisualizer}Ersetzen der QR-Codes durch Objekte in der virtuellen Realität}
Nachdem die genaue Platzierung der QR-Codes bestimmt wurde, muss ihre Visualisierung in der virtuellen Welt umgesetzt werden. Hierfür wird die Funktionalität von Unity Prefabs (siehe \ref{sec:prefabs}) genutzt, um visuelle Repräsentationen der QR-Codes zu erstellen und sie nahtlos in die virtuelle Umgebung zu integrieren.
...
\begin{figure}[H]
    \centering
    \includegraphics[scale=1, angle=0]{images/qrprefab}
...
\begin{lstlisting}[style=csharp, caption={Instanziierung eines QR-Prefabs}, label=code:QR-Prefab instanzierung]
void Start() {
    // ... weiterer Code

    // Die physikalische Groesse des QR-Codes wird abgerufen; Wird fuer Berechnungen
    // in der UpdatePropertiesDisplay() Funktion benoetigt
    PhysicalSize = qrCode.PhysicalSideLength;

    try {
        // (1) Ein neues QRItem wird erstellt und mit den Daten des gescannten
        // QR-Codes initialisiert
        ...
        // (2) Falls ein 3d-Modell gefunden wurde wird sie in der AR-Umgebung
        // sichtbar gesetzt
...
Nach der Instanziierung eines QR-Prefabs wird der Codeabschnitt \ref{code:QR-Prefab instanzierung} ausgeführt. Das Skript \textit{QRCode.cs}, in dem sich dieser Abschnitt befindet,
erhält bei der Instanziierung lediglich den Inhalt des gescannten QR-Codes übermittelt, nämlich die ID.
\begin{itemize}
    \item \textbf{Kommentar (1)}: Anhand dieser ID werden aus einer Liste möglicher Objekte die weiteren erforderlichen Informationen entnommen und innerhalb der Klasse gespeichert.
    \item \textbf{Kommentar (2)}: Zudem wird das entsprechende 3D-Modell in die Szene eingefügt.
    \item \textbf{Kommentar (3)}: Zuletzt werden die Labels mit den richtigen Werten (Name, Gewicht, Wert) befüllt, \ref{code:labels} ist der genauere Codeabschnitt.
\end{itemize}
Diese Aktionen werden bei jeder Instanziierung nur einmal ausgeführt. Im Gegensatz dazu wird die folgende Funktion (\ref{code:Aktualisieren der Position}) bei
jeder Frameaktualisierung der Brille ausgeführt und befindet sich daher in der \textit{Update()} Funktion. Weitere Informationen
zu den Lebeszyklusmethoden befinden sich in diesem Abschnitt: \ref{sec:Shrek is leben}
...
private void setLabels() {
    ...
    try {
        ...
        // Die Text-Komponenten indem sich der anzuzeigende Text befindet
        // wird geseichert
        TextMeshProUGUI nameLabel   = canvas.Find("nameLabel").gameObject
                                      .GetComponent<TextMeshProUGUI>();
        TextMeshProUGUI valueLabel  = canvas.Find("valueLabel").gameObject
                                      .GetComponent<TextMeshProUGUI>();
        TextMeshProUGUI weightLabel = canvas.Find("weightLabel").gameObject
                                      .GetComponent<TextMeshProUGUI>();

        // Die Texte der Labels werden mit den entsprechenden Daten
        // des QR-Items aktualisiert
...
void UpdatePropertiesDisplay() {
    if (qrCode != null && lastTimeStamp != qrCode.SystemRelativeLastDetectedTime
                                           .Ticks) {
        PhysicalSize = qrCode.PhysicalSideLength;
        item.qrData.position = new Vector3(PhysicalSize / 2.0f,
                                           PhysicalSize / 2.0f, 0.0f);
        labels.transform.localPosition = new Vector3(item.qrData.position.x + 0.05f,
                                                     item.qrData.position.y,
                                                     item.qrData.position.z);
        labels.transform.localScale = new Vector3(PhysicalSize * 0.02f,
                                                  PhysicalSize * 0.02f, 0.005f);
...
\begin{figure}[H]
    \centering
    \includegraphics[scale=0.2, angle=0]{images/scanned}
...
Dieser Abschnitt beschäftigt sich mit der Verwendung des Unity GameObjects \textit{Inventory Placement Controller} und
...
\begin{figure}[H]
    \centering
    \includegraphics[scale=0.35]{images/invPrefabUnity}
...
Das Unity GameObject \textit{Inventory Placement Controller} fungiert als grundlegender Baustein und Ausgangspunkt für
...
Abbildung \ref{fig:inventoryPlacementController_Editor} zeigt das GameObject im Unity Inspektor. Ebenfalls zu sehen ist
die angehängte Skript-Komponente, die dem Inventory Placement Controller GameObject zugeordnet ist. Innerhalb dieser
...
\begin{itemize}
    ...
    \item \textbf{QR Codes Manager}: Ein Verweis auf das QRCodesManager GameObject aus der Szene.
...
Die korrekte Übergabe und Konfiguration dieser Objekte und Werte an das Unity GameObject sind entscheidend für die
...
\begin{lstlisting}[caption={Funktion zur Ueberpruefung des User Gazes}, label=code:isPOP]
    ...
    if (!objectPlaced && canStartScript) {
        if (IsPointerOverPlane()) {
            ...
            if (currentPlane != null) {
                ...
                userInfo.text = ((int)requiredLookTime - (int)timeLookedAtPlane)
                                                         .ToString();
...
bzw. deaktiviert weitere GameObjects sowie Skripte, um einen nahtlosen Fortlauf der Anwendung zu gewährleisten.
...
\begin{figure}[H]
    ...
    \vspace{0.5cm} % Optionaler vertikaler Abstand zwischen den Zeilen
...
Dieser Abschnitt beschreibt das \textit{Inventory Controller} Unity GameObject und seine angehängte Skript-Komponente
...
Das Unity GameObject \texttt{Inventory Controller} ist von entscheidender Bedeutung für die nahtlose Interaktion und
...
\begin{figure}[H]
    \centering
    \includegraphics[scale=0.75]{images/invCon_Editor}
    \caption{InventoryController Objekt im Editor}
    \label{fig:InventoryController_Editor}
\end{figure}

Abbildung \ref{fig:InventoryController_Editor} zeigt das GameObject im Unity Inspektor, welches den Inventory Controller
...
\begin{itemize}
    \item \textbf{QR Code Manager}: Referenz auf das \textit{QRCodesManager} GameObject aus der Szene.

    \item \textbf{Knapsack Solver Game Objekt}: Ein Verweis auf das \textit{KnapsackSolver} GameObject aus der Szene.
...
Die korrekte Übergabe und Konfiguration dieser Objekte und Werte an das Unity GameObject ist von entscheidender Bedeutung
...
\item \texttt{GetBounds()}: Diese Methode dient dazu, die Grenzen des übergebenen GameObjects zu bestimmen. Sie verwendet
...
\subsubsection*{\label{sec:MoWeltpostionen}Lokal- und Weltposition}
...
void Update() {
    lock (activeQRObjects) {
        foreach (var item in activeQRObjects.Values) {
            ...
            Vector3 worldPosition = item.transform
                                    .TransformPoint(qRCode.item.qrData.position);
            if (item != null && inventoryBounds.Contains(worldPosition)) {
                ...
                if (!processedItems.Contains(itemId)) {
                    if (currWeight + qRCode.item.qrData.weight <= cap) {
                        ...
                        Vector2 startGridPosition =
                        CalculateGridPosition(worldPosition);
                        idGrid[(int)startGridPosition.x, (int)startGridPosition.y] =
                        itemId;
                        knapsackSolver?.UpdateInfoMesh("", Color.white);
                        currWeight += qRCode.item.qrData.weight;
                        remCap.text =
                        "Verbleibendes Gewicht: " + (cap - currWeight) + "/" + cap;
                        EventManager.GridUpdate(idGrid);
                    } else {
                        knapsackSolver?
                        .UpdateInfoMesh("Item hat zu viel Gewicht!", Color.red);
                    }
                }
            } else if (!inventoryBounds.Contains(worldPosition) &&
                       processedItems.Contains(qRCode.item.qrData.id) &&
                       ContainsId(qRCode.item.qrData.id)) {
                int itemId = qRCode.item.qrData.id;
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{images/itemAdded}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{images/itemEntfernt}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{images/itemToHeavy}
...
\newpage
\subsection{\label{sec:eventmanager}Event Manager}\marginpar{\small\(\rightarrow\) {\tiny HAYLAZ}}
...
void Start() {
    ...
    // SetInventory als OnGridUpdate Ereignishandler registrieren
...
\subsection{\label{sec:MoritzInventarBerechnen}Das Knapsack Problem} \marginpar{\small\(\rightarrow\) {\tiny SKREPEK}}
    ...
    FOR i FROM 0 TO n DO
        FOR w FROM 0 TO capacity DO
            ...
            ELSE IF weights[i-1] <= w THEN
                Tabelle[i][w] = MAX(values[i-1] + Tabelle[i-1][w - weights[i-1]],
                                    Tabelle[i-1][w])
            ELSE
...
\newpage
\subsubsection{\label{sec:moperformance}Auswahl des Implementierungsansatzes}
...
\subsubsection{\label{sec:moKnapsacksolver}Der Knapsack Solver}
Das Unity GameObject \textit{Knapsack Solver}, wie in Abbildung \ref{fig:Knapsack_Editor} dargestellt, implementiert
die Lösung des Knapsack-Problems und berechnet gleichzeitig das zusammengestellte Inventar des Benutzers. Es stellt
Feedback über diese Berechnungen bereit. Dieses GameObject mit der angehängten Skript-Komponente \textit{Knapsack Solver}
...
\begin{itemize}
    \item \textbf{QRCodesManager}: Verweis auf das \textit{QRCodesManager} GameObject aus der Szene.

    \item \textbf{Own Mesh}: Referenz auf das \textit{OwnValueMesh} GameObject aus dem Inventar Prefab

    \item \textbf{Max Mesh}: Referenz auf das \textit{MaxValueMesh} GameObject aus dem Inventar Prefab

    \item \textbf{Info Mesh}: Referenz auf das \textit{infoMesh} GameObject aus dem Inventar Prefab
...
Programmieransatzes. Die Funktion \texttt{KnapsackMaxValue()} innerhalb der Klasse \texttt{KnapsackSolver} implementiert
...
\begin{figure}[H]
    \centering
    \includegraphics[scale=0.35]{images/prefShow}
...
\begin{figure}[H]
    ...
    \caption{GameObject im Unity Editor}
    \label{fig:bestSol_Editor}
\end{figure}

Auf dieser Abbildung ist das \texttt{bestSolutionPrefab} GameObject mit den zugehörigen Komponenten in Unity selbst zu
...
\begin{itemize}
    ...
    \item \textbf{Inventory Placement Controller}: Eine Referenz auf das \\ \texttt{inventoryPlacementController} GameObject.
    \item \textbf{Knapsack Solver}: Eine Referenz auf das \textit{Knapsack Solver} GameObject.
...
und ebenfalls wird auf das Array, welches die errechnete perfekte Lösung repräsentiert (\texttt{usedItems}) zugegriffen
...
private void setNewPosition() {
    Vector3 newPosition = originalInventoryPosition + Vector3.forward * 0.395f
                          + Vector3.up * 0.16f;
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{images/perfSolUserPOV}
    \caption{Perfekte Lösung}
    \label{fig:perfSolUserPOV}
\end{figure}

\newpage
\section{Performance und Qualitätssicherung} \marginpar{\small\(\rightarrow\) {\tiny HAYLAZ}}
...
% https://www.bytehide.com/blog/attributes-csharp
...
\begin{lstlisting}[style=csharp, caption={Testfall: einzelne Gegenstaende}, label={code:einzelTest}]
...
\begin{lstlisting}[style=csharp, caption={Testfall: mehrere Gegenstaende}, label={code:mehrTest}]
...
public void KnapsackPerformanceReportItemRange() {
    ...
    for (int itemCount = 50; itemCount <= 1000; itemCount += 50) {
        // Oberen Grenzwert fuer das Gegenstandgewicht und die Kapazitaet basierend
        // auf der Gegenstandanzahl berechnen
...
\begin{lstlisting}[style=csharp, caption={Performance Messung bei 500 Aufrufen von 20 Gegenstaenden}, label={code:performanceIteration}]
[Test]
public void KnapsackPerformanceReportRangeIteration() {
    ...
    // Oberen Grenzwert fuer das Gegenstandgewicht basierend auf der
    // Gegenstandanzahl berechnen
    int weightUpperBound = itemCount * 2;

    // Schleife fuer 500 wiederholungen
    for (int i = 1; i <= 500; i++) {
        // Oberen Grenzwert fuer die Kapazitaet basierend auf der
        // Gegenstandanzahl berechnen
        int capacity = itemCount * 5;
