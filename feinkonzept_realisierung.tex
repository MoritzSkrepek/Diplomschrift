...
\begin{lstlisting}[style=csharp, caption={Code für überprüfen neuer Items im Inventar}, label=code:controller_check]
void UpdateGrid()
{
...
\end{lstlisting}
Diese Funktion ist Hauptverantworlich für die Überprüfung ob ein neues Item \textit{hinzugefügt} oder \textit{entfernt}
wurde. Zu beginn von jedem Frame wird das \textit{activeQRObjects} gelocked, weil auf dieses Object mehrere Threads zugreifen.
Anschließend wird anhand einer \textit{for-Schleife} durch jedes der Objekte in dem \textit{activeQRObjects Dictionary}
durchiteriert und von diesem Item wird mittels der \textbf{.GetComponent<QRCode>()} die QRCode Komponente dieses Objekts extrahiert
und gespeichert. Durch die Speicherung der Position des QR-Codes im Raum muss die lokale Position in eine Welt Position umgerechnet
werden, dass die Koordinaten mit dem des Inventar Objekts übereinstimmen und miteinander interagieren können.

Nach Abschluss dieses Teils beginnt der Hauptteil um zu überprüfen ob ein Item jetzt \textit{hinzugefügt} oder \textit{entfernt}
wurde. Die erste \textit{if-Abfrage} überpüft ob das aktuelle Item nicht null ist und zusätzlich auch noch ob \textbf{.Contains()}
was nur bedeutet, dass die Koordinaten des Items innerhalbs der Bounds von dem Inventar sind. Wenn dass der Fall ist
bedeutet das, dass ein Item hinzugefügt wird.

Als nächstes wird dann überprüft ob die \textit{ID} des aktuellen Items in dem \textit{processedItems} \textit{HashSet}
vorhanden ist oder nicht. Wenn die aktuelle ID noch nicht in dem \textit{HashSet} vorhanden ist, bedeutet das, dass das
aktuelle Item noch nicht verarbeitet und somit auch noch nicht hinzugefügt wurde.

Wenn das der Fall ist wird anschließend überprüft ob das aktuelle Gewicht (Das Gewicht aller momentan vorhandenen Items im
Inventar) + Das Gewicht des neuen Items kleiner ist als die \textit{Capacity}. Wenn dies der Fall ist bedeutet das, dass das
Item jetzt hinzugefügt werden kann. Hirzu wird es zu beginn in das \textit{HashSet} hinzugefügt um zu verspeichern, dass dieses
Item verarbeitet wurde. Für das visuelle für den Benutzer wird anschließend eine \textit{message} definiert die in diesem
Fall leer ist. Um dann anhand der Koordinaten innerhalb des 3D Inventar Modells die Position des Items innerhalb eines \textit{2D int arrays}
zu bestimmen wird die \textbf{CalculateGridPosition()} mit der Position des Items aufgerufen. 
